...
function FilterLabel({ tagTitle, active, toggle }: FilterLabelProps) {
  return (
    <Group>
      <Button
        classNames={{ label: styles.wrapButton }}
        h={'auto'}
        variant="light"
        p={'4'}
        onClick={toggle}
        ...
        w={'100%'}
      >
        <Checkbox color="darkGreen.4" checked={active} />
        <Space w="xs" />
        <Box w={'100%'} h={'auto'}>
...
export function FilterList({
  ...
  return (
    <Stack gap={7}>
      ...
      <FilterLabel
        tagTitle="Uncategorized"
        active={selectedTagIds.has(uncategorizedId)}
        toggle={() => toggleId(uncategorizedId)}
      />
      {content}
    </Stack>
  )
